...
export function createControl<TFieldValues extends FieldValues = FieldValues, TContext = any>(
    ...
    let _filterValues = _options.shouldUnregister ? {} : cloneObject(_defaultValues);
    ...
    const setFieldValue = (name: InternalFieldName, value: SetFieldValue<TFieldValues>) => {
        const field: Field = get(_fields, name);
        ...
        if (field) {
            ...
            if (fieldReference) {
                !fieldReference.disabled && set(_filterValues, name, getFieldValueAs(value, fieldReference));
    ...
    const setValues = <T extends InternalFieldName, K extends SetFieldValue<TFieldValues>>(name: T, value: K) => {
    ...
    const _updateDisabledField: Control<TFieldValues>['_updateDisabledField'] = ({ disabled, name }) => {
        if ((isBoolean(disabled) && _state.mount) || !!disabled || _names.disabled.has(name))
            disabled ? _names.disabled.add(name) : _names.disabled.delete(name);
    };

    const updateValue = (name: InternalFieldName, shouldSkipSetValueAs: boolean, value?: unknown, ref?: Ref) => {
        const field: Field = get(_fields, name);

        if (field) {
            const defaultValue = get(_filterValues, name, isUndefined(value) ? get(_defaultValues, name) : value);

            isUndefined(defaultValue) || (ref && (ref as HTMLInputElement).defaultChecked) || shouldSkipSetValueAs
                ? set(_filterValues, name, shouldSkipSetValueAs ? defaultValue : getFieldValue(field._f))
    ...
    const _reset: UseFilterReset<TFieldValues> = (filterValues) => {
        ...
        _filterValues = _options.shouldUnregister ? cloneObject(_defaultValues) : cloneObject(values);
    ...
    const reset: UseFilterReset<TFieldValues> = (values) =>
        _reset(isFunction(values) ? (values as Function)(_filterValues as TFieldValues) : values);
